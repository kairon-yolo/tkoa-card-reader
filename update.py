...
class CardViewer:
    def __init__(self, root):
        ...
        # Deck Builder
        ttk.Label(self.center_inner, text="Deck Builder", font=("Arial", 14, "bold")).pack(pady=5)
        self.deck_list = tk.Listbox(self.center_inner, height=5)
        self.deck_list.pack(fill="x", padx=10)

        ttk.Button(self.center_inner, text="移除選擇", command=self.remove_from_deck).pack(pady=5)
    ...
    def remove_from_deck(self):
        if not self.deck_list.curselection():
            return
        idx = self.deck_list.curselection()[0]
        self.deck_list.delete(idx)
        del self.deck[idx]
